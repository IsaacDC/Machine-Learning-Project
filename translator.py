...
# Translation only works from ENG -> JAP for now
# As we add more language options, there will be a use for origin and target language
def translate_sentence(sentence, origin_language, target_language):
    ...
    # Generates the single most probable translated token
    # More tokens generated by changing return sequences
    output_token = model.generate(input_ids, num_beams=10, num_return_sequences=1)

    return tokenizer.decode(output_token[0], skip_special_tokens=True)

# Run to test translations
# print(translate_sentence("My name is Lee"))
